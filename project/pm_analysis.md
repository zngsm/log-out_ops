...
- pm agent: codex
- date: 2026-08-09
- status: updated - Cloudflare Worker NPC API 연동 (`feat-007`), 3초 타임아웃/100% 로컬 Fallback 및 답장 완료 후 축소 시 메신저 알림 버블(`1`) 미노출 사양 반영
...
Resolved: 게임 진입 직후 비상 봉쇄 상태로 바로 들어가지 않고, **회사 인트라넷 (Diegetic 로그인 폼 `woojoo.kim`/`**********`, 메타 텍스트 전면 제거) -> [출근] -> Workstation (탭 제거, ECHO 업무 브리핑, 좌측 하단 [확인 완료] 버튼 업무 전환, 실시간 반응 대사, 동료 메신저 무작위 팝업(좌측 업무 화면 경계 내 우상단)/앱 UI(좌측 업무 화면 내 드래그 자유 이동 Draggable UI)/알림 버블(답장 전 `1`, 답장 완료 후 미노출; 좌측 업무 화면 경계 내 우하단)/자유텍스트/Unread, 이력서 서사적 판정/ECHO Q&A) -> ECHO 시스템 업데이트 승인 -> ECHO 리부팅 (CSS/SVG 글리치) -> 비상 봉쇄 & 비상 HUD/60분 타이머 가동 -> 본 퍼즐 진입** 순서로 전개한다.
...
- 기획 방향성 분석: ready (Change-001 및 Q21~Q23 사용자 피드백, Cloudflare Worker NPC API 명세 반영 완료)
- MVP 범위 확정: ready for Cloudflare Worker NPC API integrated MVP with Diegetic/ECHO-driven flow and 100% local fallback
- 전체 task 확정: `feat-001`~`feat-030` (완료), `feat-031`~`feat-034` (완료), `feat-007` (in_progress - Cloudflare Worker NPC API & 100% 로컬 Fallback)
- Active Task: `feat-007` (`src/types/npc.ts`, `src/api/npcApiClient.ts` 3초 타임아웃/로컬 Fallback, `WorkInterface.tsx` 동료 메신저 연동 & 로딩 타이핑 연출, `evidenceSubmission.ts` 및 `App.tsx` ECHO API 연동 및 스테이지 전이)
- 미확정 항목: 없음 (Q11 포함 전 항목 답변/명세 수립 완료)

## Recommended PM Decision

Cloudflare Worker NPC API 연동과 함께 3초 타임아웃 및 100% 로컬 Fallback 안전장치를 적용하는 `feat-007`을 `in_progress` 상태로 진행하여 외부 LLM 기반 대화 응답의 생동감과 오프라인/장애 환경에서의 절대적 안정성을 동시에 확보한다.
