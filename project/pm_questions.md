...
- pm agent: codex
- date: 2026-08-09
- status: Q16~Q20 answered; Change-001 specifications finalized; Q11 still blocking external AI
...
회사 인트라넷 화면은 아이디와 비밀번호가 이미 채워져 있는 로그인 폼 형태로 구성한다 (플레이어 수정/조작 불가 읽기 전용).
- 아이디: `woojoo.kim`
- 비밀번호: 10자리 마스킹 처리 (`**********`)
로그인 폼과 함께 [출근] 버튼이 노출되며, [출근] 버튼 클릭 시 2분할 업무 화면으로 진입한다.
