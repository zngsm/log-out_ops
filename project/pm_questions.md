...
## UI/UX Refinement & Polish Specification (Change-001 Follow-up v3.0 / 5 UI/UX Refinements)

### Q56. 파일 탐색기(FILE EXPLORER) 및 파일 뷰어(FILE VIEWER) 너비 비율 조절 사양

좌측 패널 내 파일 탐색기와 파일 뷰어 영역의 너비 비율 및 가독성 개선 방식에 대한 결정입니다.

### Answer

FILE EXPLORER 영역 너비를 약 180~200px 고정으로 축소하고, FILE VIEWER 영역 너비를 대폭 확장하여 로그 문서 및 텍스트 데이터의 가독성을 극대화한다.

### Q57. 인게임 주요 버튼 텍스트의 한국어 전면 전환 사양

영문으로 표기되던 인게임 주요 버튼 텍스트의 한국어 전환 사양에 대한 결정입니다.

### Answer

인게임 주요 버튼 텍스트를 다음과 같이 한국어로 전면 전환하여 다이얼로그 및 UI의 다이전틱 몰입감과 직관성을 강화한다:
...
### Q58. 잠긴 파일 열람/암호 폼 내 힌트 텍스트 전면 삭제 사양

잠긴 파일 열람 및 비밀번호 입력 폼에 노출되던 안내/힌트 문구의 처리 방식에 대한 결정입니다.

### Answer

잠긴 파일 열람/암호 폼의 `승무원 메일에서 발견된 직인 암호를 입력하십시오.` 안내 문구 및 기타 영문 힌트 문구를 전면 삭제하여 무분별한 힌트 노출을 방지하고 추리 몰입감을 극대화한다.

### Q59. 비상 HUD 크기 확충 및 상단 헤더 중앙 배치 사양

`O₂ LEVEL`, `POWER GRID`, `REMAINING TIME` 비상 HUD 카드의 시각적 표현 및 헤더 내 위치 배치에 대한 결정입니다.

### Answer

`O₂ LEVEL`, `POWER GRID`, `REMAINING TIME` 비상 HUD 카드의 시각적 크기(글자 크기, 카드 패딩, 고대비 라벨 등)를 확장하고, 상단 헤더 중앙(Center Alignment)에 시각적 균형을 이루도록 배치한다.

### Q60. `ACT-1 ACTIVE` 라벨 전면 삭제 및 비상 HUD 연동 사양

좌측 패널/헤더의 `ACT-1 ACTIVE` (또는 `STEP 1 / 5`) 라벨의 처리 및 공간 활용에 대한 결정입니다.
...
## 2 Sound/Audio Atmosphere Enhancement Specifications (Change-001 Follow-up v10.0 / feat-043)

### Q66. 2가지 사운드/오디오 분위기 강화 요구사항 (비상 봉쇄 경보음 연속 재생, 로그 제출 Phase 긴장감 BGM 자동 재생/음소거/볼륨/일시정지 연동, 오디오 톤 매칭) 상세 사양

사용자의 2가지 사운드/오디오 분위기 강화 요구사항에 대한 결정입니다.

### Answer

사용자의 2가지 사운드/오디오 분위기 강화 요구사항을 전면 반영한다:
