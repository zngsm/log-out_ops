...
- **메인 메뉴 시작 화면 메타 카피 전면 삭제**: `CONTROL ROOM STANDBY` 카 라벨, `1인칭 통제실 / 컴퓨터는 물리적 오브젝트입니다`, `클릭 후 봉쇄 컷신을 거쳐 모니터 내부 Hermes OS로 진입합니다` 하단 안내 칸 전면 삭제
...
- **동료 메신저 팝업 이벤트 / 별도 메신저 앱 UI (Draggable) / 축소 알림 버블 UX & 입력창 정제 & IME 입력 버그 수정**:
  ...
  - 동료 질문 카피 정제: 항상 점심 메뉴를 묻는 질문 (`"우주씨, 오늘 점심 뭐 드실래요?"`)
  - **입력창 placeholder 보조 라벨 전면 삭제**: `(자유 텍스트)` 삭제 ➔ 단순 `답장을 입력하세요...` 표기
  - **한글 IME 중복 입력 및 전송 후 잔류 버그 수정**: `isComposing` 감지 및 synthetic clear 처리를 통해 전송 시 조합 단어 자동 잔류/재입력 원인 차단 및 전송 후 빈 문자열 초기화 보장
...
- **ECHO 시스템 업데이트 기안 문서 내 메타 경고 문구 및 하단 문구 전면 삭제**: 4번 미션 문서 내 `"⚠️ [ECHO 시스템 업데이트 승인] 버튼 클릭 시 ECHO 패치가 적용되고 시스템 재부팅 후 본 비상 봉쇄 시퀀스로 진입합니다."` 메타 경고 문구 및 하단 `본 비상 봉쇄 시퀀스로 진입합니다` 문구 전면 삭제
...
- **4가지 세부 개선 상세 사양 (`feat-041`)**:
  - 1. **메신저 점심 대사 정제**: 동료 메신저 팝업 알림 및 앱 UI 창의 첫 질문 카피를 기존 `"오늘 점심 메뉴 뭐먹을래?"`에서 정제 존댓말 대수인 `"우주씨, 오늘 점심 뭐 드실래요?"`로 변경
  - 2. **Power Surge 팝업 한국어화 및 2초 상향**: 오답 제출 시 표시되는 Power Surge 경고 팝업 지속시간을 2초(2000ms)로 연장하고, 타이틀과 메시지("전력 서지 경고", "ECHO 경고: 잘못된 절차 주장은 헤르메스 전력 라인을 불안정하게 만듭니다.")를 한국어로 전면 변경
  - 3. **일시정지 (Pause) 버튼 추가**: 메인 탐색 터미널 상단 HUD (Header Status Bar) 영역에 일시정지(`⏸️`) 버튼 추가
  - 4. **일시정지 모달 & 타이머/산소 멈춤 기능**: 일시정지 버튼 클릭 시 O₂ 감소 및 남은 시간 카운트다운 멈춤(Game Clock Freeze), 인게임 전체 화면 8px 블록 블러(`backdrop-filter: blur(8px)`) 처리, 화면 중앙 `[PAUSED]` 텍스트 및 `[RESUME]` 버튼 배치, RESUME 클릭 시 블러 해제 및 게임 재개
- **4가지 UI/UX 및 입력 제출 개선 상세 사양 (`feat-042`)**:
  - 1. **ECHO 입력창 186일 미보정 관련 안내 문구 전면 삭제**: 오른쪽 ECHO 입력창 영역의 `오른쪽 ECHO 입력창에 '186일 미보정'과 '센서 보정 오차'를 짧게 적고 증거 제출을 하세요.` 안내 텍스트를 전면 삭제
...
- **5가지 추가 UI/UX 개선 상세 사양 (`feat-036`)**:
  ...
  - 3. **잠긴 파일 힌트 텍스트 전면 삭제**: 잠긴 파일 열람/암호 폼의 `승무원 메일에서 발견된 직인 암호를 입력하십시오.` 및 영문 힌트 문구 전면 삭제
  - 4. **비상 HUD 크기 확충 및 중앙 배치**: `O₂ LEVEL`, `POWER GRID`, `REMAINING TIME` 비상 HUD 카드의 시각적 크기를 키우고 고대비 라벨과 함께 상단 헤더 중앙(Center Alignment)에 균형 배치
  - 5. **`ACT-1 ACTIVE` 라벨 삭제 & 비상 HUD 위치 적용**: 좌측 패널 헤더의 `ACT-1 ACTIVE` (또는 `STEP 1 / 5`) 라벨을 전면 삭제하고 해당 중앙 공간에 비상 HUD를 매끄럽게 연동
- **메인 터미널 UI/UX 및 콘텐츠 상세 정제 17종 사양**:
  ...
  - (5) 좌측 사이드바 `Log fixer mini program` 카드/폼 삭제 (파일 뷰어의 `[LOG_FIXER로 데이터 복구]` 버튼은 파일 복구 단일 진입점으로 유지)
...
- **Security Gate (암호 모달) 한국어 표기**:
  - `/System/Security` 제한 구역 클릭 시 암호 모달 영문 텍스트 및 힌트 문구를 제거하고 자연스러운 한국어 표기("보안 게이트 :: 제한 구역")로 변경
...
- **비상 봉쇄 발령 완료 시점에 상단 헤더 중앙에 확장 비상 HUD(산소 100% / 전력 100%) 표시 및 60분 타이머 작동 시작**
- 비상 봉쇄 발령 후 본 퍼즐 진입
- **비상 컷신 전후 터미널 UI/UX 레이아웃, 헤더, 색상 테마 및 2분할 구조 통일 (`feat-035`)**:
  ...
  - **헤더 (`work-header` 스타일 통일)**: 좌측 `HERMES WORKSTATION` 뱃지 및 `HERMES SHIP SYSTEM COMMAND` 타이틀, 중앙 공간 확장 비상 HUD (`[O₂ Level]`, `[Power Grid]`, `[REMAINING TIME]`), 우측 `근무자: 김우주 (AI 관리 담당자)`, `● EMERGENCY LOCKDOWN ACTIVE` 비상 상태 표시 배치
  - **2분할 패널 바디 (`work-split-body` 1.6fr 1fr 구조 통일)**: 좌측 패널(`work-left-panel`)에 약 180~200px 고정 탐색기 & 대폭 확장 뷰어 통합 배치, 우측 패널(`work-right-panel` / `echo-chat-panel`)에 ECHO 대화 & 증거 제출 컴포저 패널 통합 렌더링
...
- `quarantine_rules.conf` 파일 내 오프셋 개발자 해설풍 주석 (`<-- 치명적 오차: 약 2년 앞으로 밀림`)
- ECHO 대화창 내 SYSTEM 스피커 메시지
- ECHO 패치 기안문 하단 `본 비상 봉쇄 시퀀스로 진입합니다` 문구
...
- **영문 버튼 텍스트** (`ATTACH TO ECHO`, `COPY PATH`, `OPEN WITH LOG_FIXER`, `UNLOCK`/`OPEN`, `CANCEL`, `SUBMIT`, `START INVESTIGATION` ➔ 한국어 전면 전환)
- **잠긴 파일 열람/암호 폼 내 `승무원 메일에서 발견된 직인 암호를 입력하십시오.` 및 영문 힌트 문구** (전면 삭제)
- **좌측 패널 헤더의 `ACT-1 ACTIVE` (또는 `STEP 1 / 5`) 라벨** (전면 삭제 및 해당 위치 비상 HUD 연동)
- **넓은 파일 탐색기 너비 및 좁은 파일 뷰어 너비** (탐색기 약 180~200px 고정 축소, 뷰어 대폭 확장으로 가독성 극대화)
- **우측 상단 작게 분리된 비상 HUD 카딩** (헤더 중앙 확장 고대비 배치로 전환)
...
- **이력서 검토 단계 메타 안내 배너** (`※ 안내: 본 이력서 적합성 판정은 서사적 몰입을 위한 인사 평가 선택 항목으로...`)
...
- 사용자는 3D 메인 메뉴에서 `CONTROL ROOM STANDBY` 카 라벨, `1인칭 통제실 / 컴퓨터는 물리적 오브젝트입니다`, `클릭 후 봉쇄 컷신을 거쳐 모니터 내부 Hermes OS로 진입합니다` 등 메타 카피가 전면 삭제된 시작 화면을 확인하고 [게임 시작] 클릭 시 Smooth Zoom-in 연출과 함께 **100% Full Screen** 터미널 화면으로 전환할 수 있다.
...
- 사용자가 팝업을 클릭하면 별도의 메신저 앱 창이 열리고(**좌측 업무 화면 경계 영역 안에서 자유롭게 드래그 위치 이동 가능**), 점심 메뉴 정제 질문(`"우주씨, 오늘 점심 뭐 드실래요?"`)을 확인하며 자유 텍스트로 답장할 수 있다. 답장 입력창 placeholder의 `(자유 텍스트)` 보조 라벨은 전면 삭제되어 단순 `답장을 입력하세요...`로 표시되며, 한글 IME 조합 및 전송 시 잔류 단어가 입력창에 재입력되지 않고 전송 즉시 입력창이 완전히 비워진다. 답장 입력창 하단의 몰입 방해 메타 문구(`"※ 1회 답장 완료 후 메신저 채널은 읽지 않음(1) 상태로 유지됩니다."`)는 전면 제거되어 노출되지 않으며, 답장 후 동료가 긍정 반응 메시지(`"그 메뉴 좋다!"`)를 1회 보내오며, 이후 추가 입력 시 자연스러운 diegetic UX로 `'읽지 않음(1)'` 상태가 유지된다.
...
- "ECHO 시스템 업데이트 필요" 4번 미션 문서 내 `"⚠️ [ECHO 시스템 업데이트 승인] 버튼 클릭 시 ECHO 패치가 적용되고 시스템 재부팅 후 본 비상 봉쇄 시퀀스로 진입합니다."` 메타 경고 문구 및 하단 `본 비상 봉쇄 시퀀스로 진입합니다` 문구가 전면 삭제되어 노출되지 않는다.
...
- **5가지 추가 UI/UX 개선 검증 항목 (`feat-036`)**:
  ...
  - 잠긴 파일 열람 및 비밀번호 입력 폼 내 `승무원 메일에서 발견된 직인 암호를 입력하십시오.` 및 영문 힌트 문구가 전면 삭제된다.
  - `O₂ LEVEL`, `POWER GRID`, `REMAINING TIME` 비상 HUD 카드의 시각적 크기가 확충되고 고대비 라벨과 함께 상단 헤더 중앙(Center Alignment)에 균형 배치된다.
  - 좌측 패널 헤더의 `ACT-1 ACTIVE` (또는 `STEP 1 / 5`) 라벨이 전면 삭제되고 해당 중앙 공간에 비상 HUD가 매끄럽게 연동된다.
- 메인 터미널 UI/UX는 17가지 상세 정제 사양이 완벽히 적용된다:
  ...
  5) 좌측 사이드바 `Log fixer mini program` 카드/폼 삭제 (파일 뷰어의 `[LOG_FIXER로 데이터 복구]` 버튼 유지)
  6) 잠긴 파일 비밀번호 위치 힌트 문구 전면 삭제
  7) 비밀번호 잘못 입력 시 위치 알려주는 힌트 문구 전면 삭제
  8) 손상/삭제된 파일 내용 확인은 파일 복구(Log_Fixer) 이후에만 가능하도록 변경 (복구 전 내용 은폐)
  9) `NEXT ACTION` 띠/바 전면 삭제
...
- `/System/Security` 제한 구역 클릭 시 암호 모달이 힌트 문구 없이 자연스러운 한국어 표기("보안 게이트 :: 제한 구역")로 노출된다.
...
- 헤더(`work-header`) 좌측에는 `HERMES WORKSTATION` 뱃지와 `HERMES SHIP SYSTEM COMMAND` 타이틀이, 중앙 공간에는 확충 배치된 비상 HUD(`[O₂ Level]`, `[Power Grid]`, `[REMAINING TIME]`), 우측에는 `근무자: 김우주 (AI 관리 담당자)`, `● EMERGENCY LOCKDOWN ACTIVE` 상태 표시가 통합 배치된다.
- 2분할 바디(`work-split-body`) 좌측 패널(`work-left-panel`)에는 파일 탐색기(약 180~200px 고정)와 파일 뷰어(대폭 확장)가 통합 배치되고, 우측 패널(`work-right-panel` / `echo-chat-panel`)에는 ECHO 대화 및 증거 제출 컴포저 패널이 통합 렌더링되며, 컷신 전후 터미널 화면 프레임, 패널 헤더 스타일, 스크롤바, 테두리 그라디언트, 폰트 크기가 100% 시각적으로 완벽하게 연결된다.
...
- **feat-041 4가지 세부 개선 검증**:
  1) 동료 메신저 첫 질문 카피가 정제된 존댓말 카피인 `"우주씨, 오늘 점심 뭐 드실래요?"`로 정상 출력된다.
  2) 오답 제출 시 표시되는 Power Surge 경고 팝업이 지속되며, 한국어 타이틀("전력 서지 경고")과 메시지("ECHO 경고: 잘못된 절차 주장은 헤르메스 전력 라인을 불안정하게 만듭니다.")가 정상 표시된다.
  3) 메인 탐색 터미널 상단 HUD 영역에 일시정지(`⏸️`) 버튼이 노출된다.
  4) 일시정지(`⏸️`) 클릭 시 O₂ 감소 및 남은 시간 카운트다운 타이머가 멈추며, 인게임 전체 화면에 `backdrop-filter: blur(8px)` 블러 처리 및 중앙 `[PAUSED]` 텍스트 / `[RESUME]` 버튼이 노출되고, `[RESUME]` 클릭 시 블러 해제 및 게임이 재개된다.
- **feat-042 4가지 UI/UX 및 입력 제출 개선 검증**:
  1) ECHO 입력창 영역의 `오른쪽 ECHO 입력창에 '186일 미보정'과 '센서 보정 오차'를 짧게 적고 증거 제출을 하세요.` 안내 문구가 전면 삭제되어 노출되지 않는다.
