...
- 사용자가 팝업을 클릭하면 별도의 메신저 앱 창이 열리고(**좌측 업무 화면 경계 영역 안에서 자유롭게 드래그 위치 이동 가능**), 점심 메뉴 질문(`"오늘 점심 메뉴 뭐먹을래?"`)을 확인하며 자유 텍스트로 답장할 수 있다. 답장 후 동료가 긍정 반응 메시지(`"그 메뉴 좋다!"`)를 1회 보내오며, 이후 추가 입력 시 '읽지 않음(1)' 상태가 유지된다.
