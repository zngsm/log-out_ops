...
- pm agent: codex
- date: 2026-08-10
- status: feat-033, feat-034, feat-027, feat-028, feat-030, feat-015, bug-002 updated with 5 UI/UX feedback specifications (reboot cutscene glitch -1s/error +1s & messenger bubble hide on reboot, resume fixed text '✓ [확인 완료]' & grey disabled, log explorer 3-step refinement: ACT-1 100% removed, file source <dt> removed, h2/button-bar margin unified, Security Gate Korean password modal, Blackout single center modal & Korean copy)
...
| 19 | feat-015 | feat | Add resource pressure HUD, threshold effects, and single center modal blackout popup | done | dev-agent | yes | feat-003, bug-001, bug-002 | feat-018, feat-019, qa-002 | project/tasks/feat-015.md |
...
| 42 | feat-027 | feat | Move gameplay HUD into title status bar (removing ACT-1 100% block) | done | dev-agent | no | feat-026 | human visual review | project/tasks/feat-027.md |
| 43 | feat-028 | feat | Fix compact terminal HUD and explorer layout & Security Gate Korean password modal | done | dev-agent | no | feat-027 | human visual review | project/tasks/feat-028.md |
...
| 45 | feat-030 | feat | Improve ECHO chat readability & remove log file source text | done | dev-agent | no | feat-029 | human play review | project/tasks/feat-030.md |
...
| 49 | feat-034 | feat | Implement ECHO update proposal (removing meta warning text), approval trigger, adjusted 9s CSS/SVG reboot sequence (glitch -1s / error +1s; hiding messenger bubble during reboot), and lockdown HUD/timer transition (removing title-bar ECHO STATE/monitoring & ACT-1 100% HUD) | done | dev-agent | no | feat-033 | qa-004 | project/tasks/feat-034.md |
...
- For Change-001 & 5 UI/UX feedback items, `feat-031` (CSS/SVG 2D cutscene & 100% full screen) updates the visual frame and can be tested alongside `feat-032` (diegetic intranet screen with pre-filled login and zero meta text). `feat-033` (diegetic workstation, fixed `✓ [확인 완료]` button sequential task transition with grey disabled styling `disabled={isResumeIncomplete}` for incomplete resumes, live ECHO report reactions, random colleague messenger popup (left panel top-right) with dedicated draggable app UI & minimized badge `1` at left panel bottom-right before reply / hidden after reply completion / completely hidden during reboot, free text reply for lunch menu & positive reaction, removing reply input box meta notice text & maintaining diegetic UX, applicant resume review narrative evaluation with 0 gameplay impact, meta notice banner & '서사적 판정 선택:' label removal, fixed `✓ [확인 완료]` text with grey disabled styling when unselected candidates exist, and ECHO dialogue/Q&A disabled) and `feat-034` (update proposal with zero meta warning text, approval reboot trigger, adjusted 9s reboot cutscene timeline (glitch -1s / error +1s) with messenger bubble hidden during reboot, post-reboot HUD/timer start & title-bar ECHO STATE/monitoring and `ACT-1 100%` HUD removal) form the sequential pre-puzzle narrative flow.
...
- `feat-027` keeps the resource/audio/ECHO HUD out of the bottom terminal workspace by treating it as a compact title-bar status strip and removing the `ACT-1 100%` (`mission-clock`) block.
- `feat-028` corrects the remaining visual overflow: HUD cards are no longer fixed overlays, the file explorer uses dense rows, locked files open a password modal with natural Korean text ("보안 게이트 :: 제한 구역" / "/System/Security 디렉터리는 김 박사의 비상 격리 프로토콜에 의해 잠겨 있습니다..."), file title h2 and button bar margin styles are unified, and ECHO submit remains visible.
- `feat-029` makes the first-play loop explicit with an in-world mission brief and dynamic next-action guidance.
- `feat-030` makes ECHO conversation flow readable by separating history, player submissions, ECHO answers, evidence tray, and composer controls, while completely removing the file viewer source text (`<dt>출처</dt>`).
- `feat-015` and `bug-002` ensure that when blackout (0% power) occurs, only a single center modal popup ("⚠️ [전력 고갈] 주 전력 그리드 블랙아웃! OS 터미널 긴급 재부팅 중... (남은 시간: N초)") is shown with high-contrast styling, blocking duplicate Power Surge alerts.
- Change-001 tasks (`feat-031` -> `feat-032` -> `feat-033` -> `feat-034`) refactor the game intro: 100% full screen terminal zoom -> Company Intranet with diegetic login form (`woojoo.kim` / `**********`) & Clock-in (zero meta text: `🔒 READ-ONLY`, auto auth notice, and "Clock-in :: 2분할 업무 화면 진입" removed) -> Diegetic Workstation with fixed `✓ [확인 완료]` button sequential task transition (removing "(다음 업무로 이동)" sub-label & replacing developer guide ECHO dialogue with natural diegetic copy), live ECHO report reactions, random colleague messenger popup (left panel top-right, separate draggable app UI, minimized badge `1` at left panel bottom-right before reply / hidden after reply completion / completely hidden during reboot `rebootState !== 'idle'`, free text reply for lunch menu & positive reaction, removing reply input box meta notice text & maintaining diegetic UX), candidate resume review narrative evaluation (removing meta notice banner & '서사적 판정 선택:' label, enforcing fixed `✓ [확인 완료]` text with grey disabled styling `disabled={isResumeIncomplete}` when unselected candidates exist, and ECHO dialogue/Q&A disabled) -> ECHO System Update proposal (meta warning text removed) & Reboot (CSS/SVG presentation, adjusted 9s timeline (glitch -1s / error +1s), messenger bubble hidden) -> Lockdown & Emergency HUD / 60-min timer start (title-bar ECHO STATE/monitoring and `ACT-1 100%` HUD removed) & Main Puzzle (file viewer source text `<dt>출처</dt>` removed, Security Gate Korean modal applied, Blackout single center modal applied).
