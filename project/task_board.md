...
| 48 | feat-033 | feat | Build diegetic workstation layout, simple [확인 완료] button task flow (removing '(다음 업무로 이동)' sub-label & developer guide ECHO dialogue), colleague messenger popup (left panel top-right)/draggable app UI with notification bubble (left panel bottom-right: badge 1 before reply, hidden after reply; removing reply input box meta notice text & maintaining diegetic UX), and candidate resume narrative evaluation (with ECHO dialogue/Q&A disabled) | done | dev-agent | no | feat-032 | feat-034 | project/tasks/feat-033.md |
| 49 | feat-034 | feat | Implement ECHO update proposal (removing meta warning text), approval trigger, CSS/SVG reboot sequence, and lockdown HUD/timer transition (removing title-bar ECHO STATE/monitoring HUD) | done | dev-agent | no | feat-033 | qa-004 | project/tasks/feat-034.md |
