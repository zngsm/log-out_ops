...
- For Change-001, `feat-031` (CSS/SVG 2D cutscene & 100% full screen) updates the visual frame and can be tested alongside `feat-032` (diegetic intranet screen with pre-filled login and zero meta text). `feat-033` (diegetic workstation, [확인 완료] button document transition, live ECHO reactions, random colleague messenger popup (left panel top-right) with dedicated draggable app UI & minimized badge `1` (left panel bottom-right), free-text reply for lunch menu & positive reaction, reply input box meta notice text removal & diegetic Unread handling, applicant resume review narrative evaluation with 0 gameplay impact & ECHO Q&A) and `feat-034` (update approval reboot trigger, post-reboot HUD/timer start) form the sequential pre-puzzle narrative flow.
