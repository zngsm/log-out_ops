...
- **Colleague Messenger Popup Event, Separate Draggable App UI, & Minimized Bubble Badge `1` (Left Panel Boundary Enforced)** (Q23):
  ...
  - **Minimized / Unanswered UX & New Message Bubble (Left Panel Bottom-Right)**: Player can minimize or close popup/chat window to answer later. Minimized state displays a speech-bubble chat app icon + red notification badge/bubble (`1`) bounded strictly inside the left workspace panel at bottom-right (오른쪽 하단). Clicking icon re-opens messenger app window.
...
- [x] Minimizing/closing messenger displays a speech-bubble chat icon + red notification bubble (`1`) strictly bounded within left workspace panel bottom-right (좌측 업무 화면 내 오른쪽 하단), and clicking it re-opens messenger app window.
...
- 2026-08-09: APPROVED by review_agent.
  ...
  - Colleague messenger popup (left panel top-right), dedicated draggable messenger app UI window within left panel, minimized bubble badge 1 (left panel bottom-right), free-text reply + 1-time positive reaction ("그 메뉴 좋다!"), and subsequent Unread(1) status verified.
